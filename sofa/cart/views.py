            ...
            for cart_item in cart_items:
                ...
                if cart_quantity > product_quantity:
                    messages.error(request,
                                   f"Quantity for {cart_item.product} is greater than available quantity: {product_quantity}.")
                    break
...
def checkout(request, total=0, quantity=0, cart_items=None):
    ...
    if not request.user.is_active:  # Check if the user is active
    ...
    if request.method == "POST":
        ...
        for cart_item in cart_items:
            if cart_item.quantity > cart_item.product.quantity:
                messages.error(request,
                               f"Quantity for {cart_item.product} is greater than available quantity: {cart_item.product.quantity}.")
                return redirect("user_cart")
...
def cash_on_delivery(request, order_id):
    ...
    # Check if the quantity in the cart is greater than the available quantity in the database
    cart_items = Cartitem.objects.filter(user=current_user)
    for cart_item in cart_items:
        if cart_item.quantity > cart_item.product.quantity:
            messages.warning(request, f"Quantity for {cart_item.product} is greater than available quantity: {cart_item.product.quantity}.")
            return redirect("user_cart")  # Redirect to the cart page
...
def check_product_quantities(request):
    if request.method == 'POST':
        try:
...
def wallet_pay(request, order_id):
    ...
    if applied_coupon_code:
        try:
            coupon = Coupons.objects.get(coupon_code=applied_coupon_code)
            used_coupons = UserCoupons(user=user, coupon=coupon, is_used=True)
            used_coupons.save()
        except Coupons.DoesNotExist:
            pass
